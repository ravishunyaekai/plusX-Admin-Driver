-- =============================================================================
-- All DB changes after the 03-09-2026 live update (branch: Ajo)
-- Covers commits 748c0d8 .. 6e0dcdb (04-09-2026 to 25-09-2026)
--
-- Run once on the live DB, top to bottom.
-- MySQL has no "ADD COLUMN IF NOT EXISTS" — if a statement fails with
-- "Duplicate column name" / "Duplicate key name", that change is already applied; skip it.
-- =============================================================================


-- =============================================================================
-- 1. community_managers  (08-09-2026)
-- =============================================================================

-- Table (no-op if it already exists on live)
...
-- =============================================================================
-- 3. charger_installation_inquiry  (09-09 / 14-09 / 16-09 / 18-09-2026)
-- =============================================================================

-- 09-09: optional fields made nullable
-- Still required: id, inquiry_id, customer_name, mobile_no, country_code, lead_source, enquiry_status
...
-- 16-09: link inquiry to app rider (for booking history)
ALTER TABLE charger_installation_inquiry
...
-- 16-09: backfill rider_id for existing inquiries whose mobile already has a rider account
...
-- =============================================================================
-- 4. rsa_offline_booking  (14-09 / 17-09 / 21-09-2026)
-- =============================================================================

-- 17-09: customer emirates (now required in add/edit offline RSA)
ALTER TABLE rsa_offline_booking
    ADD COLUMN emirates VARCHAR(100) NULL DEFAULT NULL AFTER address;

-- 21-09: cancellation remarks + cancelled_by (order_status = 'C')
ALTER TABLE rsa_offline_booking
...
-- Offline RSA booking now auto-creates a rider with added_from = 'Rsa Offline'
-- (old column was too short and truncated the value)
ALTER TABLE riders
    MODIFY COLUMN added_from VARCHAR(50) NULL DEFAULT NULL;

-- Migrate legacy offline labels
UPDATE riders
SET added_from = 'Rsa Offline'
WHERE added_from IN ('Admin Offline', 'Admin Offl');

-- Check before go-live: rider list now shows only added_from IN ('Android', 'iOS')
-- when no "added from" filter is passed. Any other value below will be hidden from that list.
